...
func AddHandlers(h printers.PrintHandler) {
	...
	namespaceColumnDefinitions := []metav1beta1.TableColumnDefinition{
		...
		{Name: "Age", Type: "string", Description: metav1.ObjectMeta{}.SwaggerDoc()["creationTimestamp"]},
		{Name: "Tenant", Type: "string", Description: "The tenant that the namespace belongs to"},
	}
...
func printNamespace(obj *api.Namespace, options printers.PrintOptions) ([]metav1beta1.TableRow, error) {
	...
	row.Cells = append(row.Cells, obj.Name, string(obj.Status.Phase), translateTimestampSince(obj.CreationTimestamp), obj.Tenant)
	return []metav1beta1.TableRow{row}, nil
}
...
func printControllerInstance(obj *api.ControllerInstance, options printers.PrintOptions) ([]metav1beta1.TableRow, error) {
	row := metav1beta1.TableRow{
		Object: runtime.RawExtension{Object: obj},
	}
	row.Cells = append(row.Cells, obj.Name)
...
func printControllerInstanceList(list *api.ControllerInstanceList, options printers.PrintOptions) ([]metav1beta1.TableRow, error) {
	rows := make([]metav1beta1.TableRow, 0, len(list.Items))
	for i := range list.Items {

...
func (podStrategy) TenantScoped() bool {
	return true
}
...
func PodToSelectableFields(pod *api.Pod) fields.Set {
	...
	// be adjusted.
	podSpecificFieldsSet := make(fields.Set, 11)
	podSpecificFieldsSet["spec.clusterName"] = pod.Spec.ClusterName
	...
	podSpecificFieldsSet["status.assignedScheduler.name"] = pod.Status.AssignedScheduler.Name
...
func LogLocation(
	...
	loc := &url.URL{
		...
		Path:     fmt.Sprintf("/containerLogs/%s/%s/%s/%s", pod.Tenant, pod.Namespace, pod.Name, container),
...
func PortForwardLocation(
	...
	loc := &url.URL{
		Scheme:   nodeInfo.Scheme,
		Host:     net.JoinHostPort(nodeInfo.Hostname, nodeInfo.Port),
		Path:     fmt.Sprintf("/portForward/%s/%s/%s", pod.Tenant, pod.Namespace, pod.Name),
		RawQuery: params.Encode(),
	}
	return loc, nodeInfo.Transport, nil
}

...
func (s *Scheduler) initInformers(quit chan struct{}) {
	...
	podInformer.AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			...
			util.CheckTime(pod.Name, "mockscheduler", "Run-AddFunc", 1)
			go func() {
				util.CheckTime(pod.Name, "mockscheduler", "Run-ScheduleOne", 1)
				s.ScheduleOne(pod)
				util.CheckTime(pod.Name, "mockscheduler", "Run-ScheduleOne", 2)
			}()
			util.CheckTime(pod.Name, "mockscheduler", "Run-AddFunc", 2)
		},
		UpdateFunc: func(old, new interface{}) {
...
func (s *Scheduler) ScheduleOne(p *v1.Pod) {
	...
	if p.Status.AssignedScheduler.Name == s.name {
		if clusterLen := len(s.clusters); clusterLen > 0 {
			...
			util.CheckTime(p.Name, "mockscheduler", "ScheduleOne-bindPod", 2)
			if err != nil {

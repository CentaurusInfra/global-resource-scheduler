...
func CheckIDOrName(allocation types.Allocation) error {
	if utils.IsUUIDValid(allocation.ID) != nil {
		klog.Errorf("allocation id(%#v) is invalid.", allocation.ID)
	...
	for key, value := range allocation.Stack.Labels {
		...
		if err != nil {
			klog.Errorf("allocation stack Lable key(%#v) is invalid, err: %s", key, err)
		...
		if err != nil {
			klog.Errorf("allocation stack Lable value(%#v) is invalid, err: %s", value, err)
	...
	for _, oneServer := range allocation.Stack.Resources {
		...
		if err != nil {
			klog.Errorf("server name(%#v) is invalid. err: %s", oneServer.Name, err)
			return err
		}
	}
...
func CheckStrategy(allocation types.Allocation) error {
...
func CheckFlavor(allocation types.Allocation) error {
...
// CheckStorage
func CheckStorage(allocation types.Allocation) error {
	if len(allocation.Stack.Resources) <= 0 {
		return nil
	}

...
func (sched *Scheduler) bind(ctx context.Context, stack *types.Stack, targetSiteID string,
	...
	if bindStatus.IsSuccess() {
		...
		klog.Infof("bind - podResporceAlloc: %#v", podResporceAlloc)
		klog.Infof("bind111 - sched.ResourceAllocationMap[stack.PodName]: %#v, stack.PodName:%s", sched.ResourceAllocationMap, stack.PodName)
		sched.ResourceAllocationMap[stack.PodName] = &podResporceAlloc
		/*if (sched.ResourceAllocationMap[stack.PodName] == nil) {
			sched.ResourceAllocationMap[stack.PodName] = append(sched.ResourceAllocationMap[stack.PodName], &podResporceAlloc)
		} else {
			sched.ResourceAllocationMap[stack.PodName] = &podResporceAlloc
		}*/
		klog.Infof("bind222 - sched.ResourceAllocationMap[stack.PodName]: %#v, stack.PodName:%s", sched.ResourceAllocationMap, stack.PodName)
...
func (sched *Scheduler) clusterSyncHandler(keyWithEventType KeyWithEventType) error {
	...
	startTime := time.Now()
	defer func() {
		klog.Infof("Finished syncing  %q (%v)", key, time.Since(startTime))
	}()

...
import (
	...
	klog "k8s.io/klog"
	scheme "k8s.io/kubernetes/globalscheduler/pkg/apis/cluster/client/clientset/versioned/scheme"
...
type ClustersGetter interface {
	...
	ClustersWithMultiTenancy(namespace string, tenant string) ClusterInterface
}
...
func newClusters(c *GlobalschedulerV1Client, namespace string) *clusters {
	return newClustersWithMultiTenancy(c, namespace, "system")
}

func newClustersWithMultiTenancy(c *GlobalschedulerV1Client, namespace string, tenant string) *clusters {
	return &clusters{
		client:  c.RESTClient(),
...
func (c *clusters) List(opts metav1.ListOptions) (result *v1.ClusterList, err error) {
	...
	if wgLen > 1 {
		...
		wg.Add(wgLen)
		results := make(map[int]*v1.ClusterList)
		errs := make(map[int]error)
		for i, client := range c.clients {
			go func(c *clusters, ci rest.Interface, opts metav1.ListOptions, lock *sync.Mutex, pos int, resultMap map[int]*v1.ClusterList, errMap map[int]error) {
				r := &v1.ClusterList{}
				err := ci.Get().
		...
		itemsMap := make(map[string]*v1.Cluster)
		for j := 0; j < wgLen; j++ {
...
func (c *clusters) Watch(opts metav1.ListOptions) watch.AggregatedWatchInterface {
	var timeout time.Duration
	if opts.TimeoutSeconds != nil {
		timeout = time.Duration(*opts.TimeoutSeconds) * time.Second
	}

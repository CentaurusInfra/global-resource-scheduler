...
type ClusterLister interface {
	...
	ClustersWithMultiTenancy(namespace string, tenant string) ClusterNamespaceLister
...
func (s *clusterLister) Clusters(namespace string) ClusterNamespaceLister {
	return clusterNamespaceLister{indexer: s.indexer, namespace: namespace, tenant: "system"}
}

func (s *clusterLister) ClustersWithMultiTenancy(namespace string, tenant string) ClusterNamespaceLister {
	return clusterNamespaceLister{indexer: s.indexer, namespace: namespace, tenant: tenant}
}

// ClusterNamespaceLister helps list and get Clusters.
type ClusterNamespaceLister interface {
	// List lists all Clusters in the indexer for a given tenant/namespace.
	List(selector labels.Selector) (ret []*v1.Cluster, err error)
	// Get retrieves the Cluster from the indexer for a given tenant/namespace and name.

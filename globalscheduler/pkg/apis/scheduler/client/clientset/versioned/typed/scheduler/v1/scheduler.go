...
import (
	...
	klog "k8s.io/klog"
	scheme "k8s.io/kubernetes/globalscheduler/pkg/apis/scheduler/client/clientset/versioned/scheme"
...
type SchedulersGetter interface {
	...
	SchedulersWithMultiTenancy(namespace string, tenant string) SchedulerInterface
}
...
func newSchedulers(c *GlobalschedulerV1Client, namespace string) *schedulers {
	return newSchedulersWithMultiTenancy(c, namespace, "system")
}

func newSchedulersWithMultiTenancy(c *GlobalschedulerV1Client, namespace string, tenant string) *schedulers {
	return &schedulers{
		client:  c.RESTClient(),
...
func (c *schedulers) List(opts metav1.ListOptions) (result *v1.SchedulerList, err error) {
	...
	if wgLen > 1 {
		...
		wg.Add(wgLen)
		results := make(map[int]*v1.SchedulerList)
		errs := make(map[int]error)
		for i, client := range c.clients {
			go func(c *schedulers, ci rest.Interface, opts metav1.ListOptions, lock *sync.Mutex, pos int, resultMap map[int]*v1.SchedulerList, errMap map[int]error) {
				r := &v1.SchedulerList{}
				err := ci.Get().
		...
		itemsMap := make(map[string]*v1.Scheduler)
		for j := 0; j < wgLen; j++ {
...
func (c *schedulers) Watch(opts metav1.ListOptions) watch.AggregatedWatchInterface {
	var timeout time.Duration
	if opts.TimeoutSeconds != nil {
		timeout = time.Duration(*opts.TimeoutSeconds) * time.Second
	}

...
func newRolesWithMultiTenancy(c *RbacV1Client, namespace string, tenant string) *roles {
	return &roles{
		client:  c.RESTClient(),
...
func (c *roles) List(opts metav1.ListOptions) (result *v1.RoleList, err error) {
	...
	if wgLen > 1 {
		...
		itemsMap := make(map[string]*v1.Role)
		for j := 0; j < wgLen; j++ {
...
func (c *roles) Watch(opts metav1.ListOptions) watch.AggregatedWatchInterface {
	var timeout time.Duration
	if opts.TimeoutSeconds != nil {
		timeout = time.Duration(*opts.TimeoutSeconds) * time.Second
	}

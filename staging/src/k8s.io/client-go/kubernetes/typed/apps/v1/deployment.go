...
func newDeploymentsWithMultiTenancy(c *AppsV1Client, namespace string, tenant string) *deployments {
	return &deployments{
		client:  c.RESTClient(),
...
func (c *deployments) List(opts metav1.ListOptions) (result *v1.DeploymentList, err error) {
	...
	if wgLen > 1 {
		...
		itemsMap := make(map[string]*v1.Deployment)
		for j := 0; j < wgLen; j++ {
...
func (c *deployments) Watch(opts metav1.ListOptions) watch.AggregatedWatchInterface {
	var timeout time.Duration
	if opts.TimeoutSeconds != nil {
		timeout = time.Duration(*opts.TimeoutSeconds) * time.Second
	}
